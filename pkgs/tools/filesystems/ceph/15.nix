...
in rec {
  ...
  ceph-client-15 = runCommand "ceph-client-${version}" {
      meta = getMeta "Tools needed to mount Ceph's RADOS Block Devices";
    } ''
      mkdir -p $out/{bin,etc,${sitePackages},share/bash-completion/completions}
      cp -r ${ceph-15}/bin/{ceph,.ceph-wrapped,rados,rbd,rbdmap} $out/bin
      cp -r ${ceph-15}/bin/ceph-{authtool,conf,dencoder,rbdnamer,syn} $out/bin
      cp -r ${ceph-15}/bin/rbd-replay* $out/bin
      cp -r ${ceph-15}/${sitePackages} $out/${sitePackages}
      cp -r ${ceph-15}/etc/bash_completion.d $out/share/bash-completion/completions
      # wrapPythonPrograms modifies .ceph-wrapped, so lets just update its paths
      substituteInPlace $out/bin/ceph          --replace ${ceph-15} $out
      substituteInPlace $out/bin/.ceph-wrapped --replace ${ceph-15} $out
   '';
}
